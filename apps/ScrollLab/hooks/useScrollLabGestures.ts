...
type GestureId = TransitionId | 'system.back';

export function useScrollLabGestures() {
  ...
  const bindBack = <T extends HTMLElement>(
    options?: (TriggerTapOptions<T> & { steps?: number }),
  ) => {
  ...
  return { bindTap, bindLongPress, bindDoubleTap, bindBack, go, back, navigateTo };
}
