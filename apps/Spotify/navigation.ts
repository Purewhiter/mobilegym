...
export function useAppNavigate() {
  ...
  const go = useCallback(
    (
      ...
      options?: { state?: unknown; mode?: 'push' | 'replace'; popTo?: string; popToInclusive?: boolean },
    ) => {
    ...
    [location.pathname, location.search, navigate, searchParams, navigator],
  );
...
function matchFrom(from: TransitionDeclaration['from'], pathname: string, searchParams: URLSearchParams) {
  if (from === '*') return true;
...
function buildSearchParams(options: {
  ...
  const { currentSearchParams, preserveParams, staticSearch, dynamicSearchParams, runtimeParams } = options;

  const newParams = new URLSearchParams();

  // 1) 保留指定的当前参数（用于“保持 tab / 记忆”之类）
  ...
  // 3) 应用动态 searchParams（从 runtime params 取值）
...
function resolveValue(
  ...
  // appState 未接入：保持 null（cases 目前不依赖 appState）
  return null;
}
