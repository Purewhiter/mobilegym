...
- **`[score-affecting]` Weather: single-city prompts now say `{city}这个城市…`.** The Weather home page shows the located point (海淀区 中关村街道), which is itself in Beijing, while `city="北京"` is judged against the saved 北京市 card. With the bare `{city}…` wording, an agent answering from the home page had a defensible reading, so a failure could not be cleanly attributed to the agent. The new wording points at the city-level entry without hinting at the city list — telling the located point from the city is still part of what these tasks test. Judge logic is unchanged. Affects `weather` / `CheckCurrentTemp`, `CheckCurrentWeather` ([#27](https://github.com/Purewhiter/mobilegym/pull/27), thanks @Lxr-max), `CheckDetailCard`, `OpenDailyForecast`, `CheckAQIPollutant`, `CheckLifeIndex`, `WarmestDayInWeek`, `SwitchUnitAndReport`, `FeelsLikeDiff`, `ColdestDayIn14`, `NightLowTemp`, `ConditionalAction`. Cross-app tasks that mention a city (`crossapp_life`) are deliberately left as they are: their wording already names a destination. Reported in [#16](https://github.com/Purewhiter/mobilegym/issues/16).
- **Parallel runner crash on setup failure with `repeat_n > 1`.** The `worker` closure in `_run_with_repeat` did `fail_count += skipped` without declaring `nonlocal`, so accounting for the skipped trials raised `UnboundLocalError` and killed the worker. Score-neutral: the path always crashed before. ([#24](https://github.com/Purewhiter/mobilegym/pull/24), thanks @Anai-Guo)
